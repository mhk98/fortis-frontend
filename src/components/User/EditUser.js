...
import { useGetUserDataByIdQuery, useUpdateUserMutation } from '../../features/auth/auth';
...
const {
  ...
  try {
    const res = await updateUser({id, data:formData});
    if (res.data?.success) {
...
const { data, isLoading, isError, error } = useGetUserDataByIdQuery(id);
...
useEffect(() => {
    if (isError) {
      ...
      reset({
        ...
        Property: fetchedUser.Property || '',
        Status: fetchedUser.Status || '',
        Password: '',            // don't autofill passwords
        ConfirmPassword: '',     // don't autofill passwords
      });
    }
      ...
      <div className="p-6">
        <div className="flex flex-col gap-6">
          <form onSubmit={handleSubmit(onFormSubmit)}>
            ...
            <div className="flex flex-col items-start w-full">
              <label className="font-medium mb-2">Current Image:</label>
              <div className="w-32 h-32 border border-gray-300 mb-2 bg-gray-100 flex items-center justify-center overflow-hidden">
                
              {(user.image) ? (
                  <img src={`http://localhost:5000/${user.image}`} alt="Preview" className="w-full h-full object-cover" />
                ) : (
                  <span />
                )}
